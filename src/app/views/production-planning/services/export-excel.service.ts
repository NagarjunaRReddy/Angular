...
export class ExportExcelService {
  ...
  exportExcel(excelData:any) {
    ...
    var date = new Date();
    //Footer Row
    let footerRow = worksheet.addRow(['Report Generated On  ' + this.datepipe.transform(date, 'MM-dd-yy')]);
    ...
    if (value && typeof value === 'string' && value.includes("color:")) {
      ...
      if (colorMatch && colorMatch[1]) {
        ...
        if(value.includes('| color:')){
          cellValue = value.replace(/(\s*\|\s*color:\s*#[0-9A-Fa-f]{6})|#([0-9A-Fa-f]{6})/, '').trim();
        }else{
...
exportExcelForReports(excelData) {
  ...
  data.forEach(d => {
    ...
    if (this.ConvertDate(axDeliveryDate) != this.ConvertDate(sandopDeliveryDate)) {
